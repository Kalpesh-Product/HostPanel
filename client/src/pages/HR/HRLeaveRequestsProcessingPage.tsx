    ...
    <div className="p-2 lg:p-2.5 min-h-full text-[#0F172A] font-sans text-[12px]">
              ...
              <div className="flex flex-col items-start justify-between gap-3 border-b border-slate-100/60 bg-slate-50/50 p-3 sm:gap-4 sm:p-4 xl:flex-row xl:items-center lg:p-5">
                ...
                <div className="flex w-full flex-wrap items-center gap-3 sm:flex-nowrap xl:w-auto">
                  ...
                  <div className="relative min-w-[180px] flex-1">
                    <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={15} />
                    <input data-tour="hr-leave-search" type="text" placeholder="Search name or role..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-lg border border-slate-200/60 bg-white py-2.5 pl-9 pr-4 text-[12px] font-pmedium text-[#0F172A] outline-none transition-all placeholder:text-slate-500 focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/20" />
                  </div>
                </div>
              </div>
            )}
      ...
      <AnimatePresence>
        {isLeaveTypeModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex items-center justify-center bg-[#0F172A]/70 p-4 backdrop-blur-sm">
      ...
      <AnimatePresence>
        {assignmentEmployee && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[112] flex items-center justify-center bg-[#0F172A]/70 p-4 backdrop-blur-sm">
      ...
      <AnimatePresence>
        {viewingCalendarEntry && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[115] flex items-center justify-center bg-[#0F172A]/55 p-4 backdrop-blur-sm">
      ...
      <AnimatePresence>
        {isCalendarEntryModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[116] flex items-center justify-center bg-[#0F172A]/55 p-4 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.96, opacity: 0 }} className="w-full max-w-lg overflow-hidden rounded-[1.75rem] border border-white/70 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleHolidaySubmit} className="space-y-4 p-5 font-pmedium">
                ...
                {holidayForm.entryKind === "event" && <div><label className="mb-1.5 block text-[10px] uppercase tracking-widest text-slate-500">Location</label><div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={14} /><input value={holidayForm.location} onChange={(e) => setHolidayForm((prev) => ({ ...prev, location: e.target.value }))} placeholder="Event venue or meeting point" className="w-full rounded-xl border border-slate-200 py-2.5 pl-9 pr-3 text-[12px] outline-none transition-all placeholder:text-slate-500 focus:border-[#2563EB] focus:ring-2 focus:ring-blue-100" /></div></div>}
