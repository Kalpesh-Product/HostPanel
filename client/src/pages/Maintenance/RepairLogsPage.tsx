  ...
  const handleExportReport = async ({ format, dataWindow, period, reportMonth, dateFrom, dateTo }: ExportParams) => {
    ...
    const exportRows = visibleLogs
      ...
      value: [
        ...
        log.sourceTicketCode ? `Ticket: ${log.sourceTicketCode}` : "",
      ].filter(Boolean).join(" | "),
      }));
    ...
    try {
      const response = await createReport({
        ...
        generatedBy: currentUserName || "Maintenance Manager",
        format: reportFormat,
        description: `Maintenance repair work orders for the current view (${activeTab}).`,
        sourceType: "custom",
        sourceRef: "maintenance-repair-logs",
      ...
      toast.error(e?.response?.data?.message || e?.message || "Failed to export repair log report.");
    }
  };
    ...
    <div className="p-2 lg:p-2.5">
      <PageFrame>
        <div className="flex flex-col gap-4">
          ...
          {/* Pill Tabs */}
          <div data-tour="maintenance-repair-tabs" data-active-tab={activeTab} className="mb-3 flex flex-wrap gap-1.5 rounded-2xl border border-slate-100 bg-white p-1 shadow-sm">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
          ...
          {/* Stat Cards */}
          <div data-tour="maintenance-repair-stats" className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3 shrink-0">
            {[
              ...
              const labelColors = ['text-slate-400', 'text-amber-600', 'text-blue-600', 'text-emerald-600'];
              return (
                <div key={card.key} className={`bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm flex justify-between items-center transition-all hover:shadow-md ${borderColors[idx] || ''}`}>
                  <div className="min-w-0">
                    <p className={`text-[10px] font-pmedium ${labelColors[idx] || 'text-slate-400'} uppercase tracking-widest mb-1`}>{card.label}</p>
          ...
          <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
            ...
            <div className="p-3 sm:p-4 lg:p-5 border-b border-slate-100/60 flex flex-col xl:flex-row justify-between items-center gap-4 bg-slate-50/50">
              <div data-tour="maintenance-repair-status-filter" className="flex items-center gap-3">
                <select
                  value={statusFilter}
              ...
              <div data-tour="maintenance-repair-search-create" className="flex items-center gap-3 w-full xl:w-auto">
            ...
            {/* Table */}
            <div data-tour="maintenance-repair-table" className="overflow-x-auto">
      ...
      <ExportReportModal
        ...
        title="Export Repair Logs"
        subtitle="Select format and date range to export."
        department="Maintenance"
        category="RepairLogs"
        sourceRef="maintenance-repair-logs"
        reportTitle="Maintenance Repair Logs"
