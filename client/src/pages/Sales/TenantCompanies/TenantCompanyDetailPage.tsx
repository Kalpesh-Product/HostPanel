...
function fmtDate(v) {
  if(!v)return'-';const d=new Date(v);return isNaN(d.getTime())?v:new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'short',year:'numeric'}).format(d);
}
...
export default function TenantCompanyDetailPage() {
  ...
  const ca = useMemo(()=>Number(tenant?.packageDetails?.monthlyTotalCredits||tenant?.creditsAllocated||tenant?.creditsTotal||0),[tenant]);
  const pc = useMemo(()=>Number(tenant?.addOnCredits?.purchasedCredits||0),[tenant]);
            ...
            {activeTab==='company-details'&&(
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    <h3 className="text-xs font-black uppercase tracking-wider text-slate-900 border-b border-slate-100 pb-3 mb-4 flex items-center gap-2"><DollarSign size={14}/> Billing Snapshot</h3>
                ...
                {tenant.agreementDetails && Object.keys(tenant.agreementDetails).length > 0 && (
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {Object.entries(tenant.agreementDetails).filter(([k]) => k !== '_id').map(([key, val]) => (
                        <div key={key}>
                          <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{key.replace(/([A-Z])/g,' $1').replace(/^./,s=>s.toUpperCase())}</p>
                          <p className="text-xs font-bold text-slate-900 mt-1">{String(val||'N/A')}</p>
                        </div>
                      ))}
                    </div>
                  </div>
