...
const AccessProfile = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await axios.post(`/api/access/modify-permissions/${user._id}`, {
  ...
  const groupPermissionsByModule = (permissionsObj: any) => {
    const grouped: Record<string, any[]> = {};
    Object.entries(permissionsObj).forEach(([key, { value, type, access }]: any) => {
  ...
  if (isPending) {
    return <div className="flex justify-center items-center h-screen"><p className="text-lg font-semibold">Loading user permissions...</p></div>;
  }

  if (isError) {
    return <div className="flex justify-center items-center h-screen"><p className="text-lg font-semibold text-red-500">Failed to load permissions.</p></div>;
  }
          ...
          <div className="flex flex-col gap-6">
            <span className="text-title flex items-center gap-3">
              ...
              <Chip
                ...
                sx={{ backgroundColor: user.status ? "green" : "grey", color: "white" }}
              />
            </span>
        ...
        {Object.entries(groupedPermissions).map(([module, permissions]) => (
          <div key={module}>
            ...
            >
              ...
              <span className="text-content">{permissions.length} permissions</span>
            </div>
          </div>
        ))}
      </div>
