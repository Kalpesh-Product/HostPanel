...
interface PrintoutRequestRecord {
  ...
  status: string;
  requestedByUserId?: { _id?: string; id?: string } | string | null;
...
export default function PrintoutsPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (editingRequestId) {
        ...
        toast.success('Printout request submitted');
      }
      closeRequestModal();
      loadRequests();
    } catch (err: any) {
  ...
  const isOwnPendingRequest = (request: PrintoutRequestRecord) => {
    if (request.status !== 'Pending') return false;
    const owner = request.requestedByUserId as any;
