...
export function ReportsPage({ embedded = false }: ReportsPageProps = {}) {
    ...
    return [
      {
        ...
        icon: <Layers size={16} />, iconClass: 'bg-slate-100 text-slate-600', labelClass: 'text-slate-400', borderClass: '',
      },
      {
        label: 'This Month', value: thisMonth,
        icon: <Calendar size={16} />, iconClass: 'bg-blue-50 text-blue-600', labelClass: 'text-blue-600', borderClass: 'border-l-4 border-l-blue-500',
      },
      {
        label: 'Previous Month', value: prevMonth,
        icon: <Calendar size={16} />, iconClass: 'bg-amber-50 text-amber-600', labelClass: 'text-amber-600', borderClass: 'border-l-4 border-l-amber-500',
      },
      {
        label: 'PDF Reports', value: pdfCount,
        icon: <FileDown size={16} />, iconClass: 'bg-red-50 text-red-600', labelClass: 'text-red-600', borderClass: 'border-l-4 border-l-red-500',
      },
    ];
  }, [reports, thisMonthKey, prevMonthKey]);

  const isViewingExcelReport = String(viewingReport?.format || '').toLowerCase() === 'excel';
  const viewingCatStyle = viewingReport ? getCategoryStyle(viewingReport.category) : null;
  ...
  const pageContent = (
    <>
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
          ...
          <div data-tour="reports-filters" className="flex flex-col sm:flex-row flex-wrap gap-2 w-full xl:w-auto">
            ...
            <div className="relative w-full sm:w-auto">
              <select value={selectedDepartment} onChange={(e) => setSelectedDepartment(e.target.value)}
                className="w-full sm:w-auto pl-3.5 pr-8 py-2.5 bg-blue-50/50 hover:bg-blue-50 border border-blue-100 rounded-lg text-[10px] font-pmedium uppercase tracking-widest text-[#2563EB] outline-none appearance-none cursor-pointer transition-all shadow-sm">
          ...
          <table data-tour="reports-table" className="w-full min-w-[1120px] text-left font-pmedium">
            ...
            <tbody className="divide-y divide-slate-100/60">
              {filteredReports.length === 0 ? (
                <tr><td colSpan={9} className="py-16 text-center font-pmedium text-slate-400">No reports found matching your criteria.</td></tr>
                  ...
                  <tr key={report.id} className="group hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="font-pmedium text-[#0F172A] text-[13px]">{report.title}</p>
                          <p className="text-[11px] font-pmedium text-slate-400">{report.size}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2 font-pmedium text-slate-900">
                        ...
                        <span className="text-[12px]">{report.generatedBy}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 text-[12px] font-pmedium text-slate-600">
                        <Calendar size={13} className="text-slate-400" /> {report.period}
                      </div>
                    </td>
                    <td className="px-5 py-4"><span className="text-[12px] font-pmedium text-slate-500">{report.generatedDate}</span></td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-center gap-1.5">
                        <button onClick={() => setViewingReport(report)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:bg-blue-100 hover:text-blue-700" title="View Details">
                          <Eye size={15} strokeWidth={2.5} />
                        </button>
                        <button onClick={() => handleDownload(report)} disabled={isDownloadingReportId === report.recordId}
                          className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:bg-emerald-100 hover:text-emerald-600 disabled:opacity-60 disabled:cursor-not-allowed" title={`Download ${report.format}`}>
        ...
        <div data-tour="reports-table" className="md:hidden flex flex-col p-4 gap-4 bg-slate-50/30">
          {filteredReports.map((report) => {
            const catStyle = getCategoryStyle(report.category);
            return (
              <div key={report.id} className="bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm flex flex-col gap-3 font-pmedium">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-pmedium text-[#0F172A] text-sm leading-tight">{report.title}</p>
                      <p className="text-[11px] font-pmedium text-slate-500 flex items-center gap-1"><UserCheck size={11} className="text-slate-400" />{report.generatedBy}</p>
                    </div>
                  </div>
                ...
                <div className="text-[11px] font-pmedium text-slate-500">Department: <span className="text-slate-700">{report.department || 'General'}</span></div>
                <div className="grid grid-cols-3 gap-2 bg-slate-50 rounded-xl p-3 border border-slate-100 text-xs font-pmedium text-slate-600">
                  <div><span className="text-[10px] text-slate-400 uppercase tracking-wider font-pmedium block mb-0.5">Period</span>{report.period}</div>
                  <div><span className="text-[10px] text-slate-400 uppercase tracking-wider font-pmedium block mb-0.5">Format</span><span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-pmedium ${getFormatStyle(report.format)} border`}>{report.format}</span></div>
                  <div><span className="text-[10px] text-slate-400 uppercase tracking-wider font-pmedium block mb-0.5">Size</span>{report.size}</div>
                  <div><span className="text-[10px] text-slate-400 uppercase tracking-wider font-pmedium block mb-0.5">Window</span>{report.dataWindow || 'Monthly'}</div>
                </div>
                <div className="flex justify-end gap-2 pt-1 border-t border-slate-100">
                  <button onClick={() => setViewingReport(report)} className="flex-1 py-2 bg-white border border-slate-200 text-slate-600 font-pmedium text-xs rounded-xl shadow-sm">View</button>
                  <button onClick={() => handleDownload(report)} disabled={isDownloadingReportId === report.recordId}
                    className="flex-1 py-2 bg-emerald-50 border border-emerald-200 text-emerald-700 font-pmedium text-xs rounded-xl shadow-sm flex items-center justify-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed">
          ...
          {filteredReports.length === 0 && (
            <div className="py-20 text-center text-sm font-pmedium text-slate-400">No reports found.</div>
          )}
        </div>

        {pagination?.hasNextPage && (
          <div className="flex flex-col items-center gap-2 border-t border-slate-100/70 bg-white/70 px-4 py-5">
            <p className="text-[12px] font-pmedium text-slate-500">Showing {reports.length} of {pagination?.total || reports.length} reports.</p>
      ...
      {viewingReport && viewingCatStyle ? (
        <div className="fixed inset-0 bg-[#0F172A]/40 backdrop-blur-sm flex items-center justify-center z-50 p-3" onClick={() => setViewingReport(null)}>
          <div className="bg-white rounded-[2rem] max-w-xl w-full shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-white/70 max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 sm:p-6 border-b border-slate-100 bg-blue-50/30 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-base lg:text-lg font-pmedium tracking-tight text-slate-800 truncate">{viewingReport.title}</h2>
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <span className={`px-2.5 py-1 rounded-md text-[9px] font-pmedium uppercase tracking-wider ${viewingCatStyle.bg} ${viewingCatStyle.text} border ${viewingCatStyle.border}`}>{viewingReport.category}</span>
              ...
              <button onClick={() => setViewingReport(null)} className="w-8 h-8 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 shadow-sm hover:text-slate-700 hover:bg-slate-50 transition-colors shrink-0">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-5 sm:p-6 space-y-5 overflow-y-auto flex-1 bg-white">
              <div>
                <h3 className="text-[10px] font-pmedium text-slate-500 uppercase tracking-widest border-b border-slate-100 pb-2 mb-3 flex items-center gap-2">
                  <FileText size={14} /> Report Information
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50/60 p-4 rounded-2xl border border-slate-100">
                  <div>
                    ...
                    <p className="text-[12px] font-pmedium text-slate-900">{viewingReport.department || 'General'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-slate-500 uppercase font-pmedium tracking-widest mb-1">Data Window</p>
                    <p className="text-[12px] font-pmedium text-slate-900">{viewingReport.dataWindow || 'Monthly'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-slate-500 uppercase font-pmedium tracking-widest mb-1">Report Month</p>
                    <p className="text-[12px] font-pmedium text-slate-900">{formatMonthLabel(viewingReport.reportMonth || deriveReportMonthKey(viewingReport)) || 'Not set'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-slate-500 uppercase font-pmedium tracking-widest mb-1">Generated By</p>
                    <p className="text-[12px] font-pmedium text-slate-900 flex items-center gap-1.5"><UserCheck size={13} className="text-slate-400" />{viewingReport.generatedBy}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-slate-500 uppercase font-pmedium tracking-widest mb-1">Period</p>
                    <p className="text-[12px] font-pmedium text-slate-900 flex items-center gap-1.5"><Calendar size={13} className="text-slate-400" />{viewingReport.period}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-slate-500 uppercase font-pmedium tracking-widest mb-1">Date</p>
                    <p className="text-[12px] font-pmedium text-slate-500">{viewingReport.generatedDate}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-slate-500 uppercase font-pmedium tracking-widest mb-1">Size</p>
                    <p className="text-[12px] font-pmedium text-slate-900">{viewingReport.size}</p>
                  </div>
                </div>
              </div>

              {isViewingExcelReport && (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-pmedium text-emerald-800">
              ...
              {viewingReport.description && (
                <div>
                  ...
                  <div className="p-4 bg-slate-50/60 border border-slate-100 rounded-2xl italic font-pmedium text-slate-700 text-[13px] leading-relaxed">&ldquo;{viewingReport.description}&rdquo;</div>
                </div>
              )}

              {Array.isArray(viewingReport.monthlyData) && viewingReport.monthlyData.length > 0 && (
                <div>
                  <h3 className="text-[10px] font-pmedium text-slate-500 uppercase tracking-widest border-b border-slate-100 pb-2 mb-3">Monthly Data</h3>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {viewingReport.monthlyData.map((item, index) => (
                          <tr key={`${item.month}-${item.metric}-${index}`}>
                            <td className="px-3 py-2 text-xs font-pmedium text-slate-600">{item.month}</td>
                            <td className="px-3 py-2 text-xs font-pmedium text-slate-800">{item.metric}</td>
                            <td className="px-3 py-2 text-xs font-pmedium text-blue-700">{item.value}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {Array.isArray(viewingReport.reportRows) && viewingReport.reportRows.length > 0 && (
                <div>
                  <h3 className="text-[10px] font-pmedium text-slate-500 uppercase tracking-widest border-b border-slate-100 pb-2 mb-3">Report Details</h3>
                  <div className="border border-slate-200 rounded-2xl overflow-hidden">
                    <table className="w-full text-left">
                      <thead className="bg-slate-50 text-[10px] font-pmedium text-slate-500 uppercase tracking-widest">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {viewingReport.reportRows.map((item, index) => (
                          <tr key={`${item.label}-${index}`}>
                            <td className="px-3 py-2 text-xs font-pmedium text-slate-600">{item.label}</td>
                            <td className="px-3 py-2 text-xs font-pmedium text-slate-800">{item.value}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            {/* Footer */}
            <div className="p-5 sm:p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-2">
              <div className={`grid grid-cols-1 ${isViewingExcelReport ? '' : 'sm:grid-cols-2'} gap-3 w-full`}>
                {!isViewingExcelReport && (
                  <button onClick={() => handleViewReport(viewingReport)}
                    className="w-full py-3 bg-white border border-slate-200 text-[#0F172A] rounded-xl font-pmedium text-[13px] uppercase tracking-wider shadow-sm hover:bg-slate-50 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
                    <Eye size={16} /> View
                  </button>
                )}
      ...
      {showDownloadPicker && (
        <div className="fixed inset-0 bg-[#0F172A]/40 backdrop-blur-sm flex items-center justify-center z-50 p-3" onClick={() => setShowDownloadPicker(false)}>
          <div className="bg-white rounded-[2rem] max-w-xl w-full shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-white/70 max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="p-5 sm:p-6 border-b border-slate-100 bg-blue-50/30 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-base lg:text-lg font-pmedium tracking-tight text-slate-800 truncate">Download Reports</h2>
                  <p className="text-[11px] font-pmedium text-slate-500 mt-1">Select department, pick a report, and choose format.</p>
                </div>
              </div>
              <button onClick={() => setShowDownloadPicker(false)} className="w-8 h-8 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 shadow-sm hover:text-slate-700 hover:bg-slate-50 transition-colors shrink-0">
                <X size={16} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 gap-4 bg-slate-50/60 p-4 rounded-2xl border border-slate-100">
                ...
                <PickerField label="Report">
                  <select value={selectedDepartmentReportId} onChange={(e) => setSelectedDepartmentReportId(e.target.value)}
                    ...
                    className="w-full pl-4 pr-10 py-2.5 bg-white border border-slate-200/60 rounded-lg font-pmedium text-[12px] text-[#0F172A] focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB] outline-none appearance-none disabled:opacity-60 disabled:cursor-not-allowed">
                  ...
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={16} />
                  {isLoadingDepartmentReports && <p className="text-[11px] font-pmedium text-blue-600">Loading department reports...</p>}
                </PickerField>
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setDownloadPickerFormat('PDF')}
                      className={`py-2.5 rounded-xl border font-pmedium text-xs uppercase tracking-widest transition-all ${downloadPickerFormat === 'PDF' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-white border-slate-200 text-slate-600'}`}>PDF</button>
                    <button onClick={() => setDownloadPickerFormat('Excel')}
                      className={`py-2.5 rounded-xl border font-pmedium text-xs uppercase tracking-widest transition-all ${downloadPickerFormat === 'Excel' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-slate-200 text-slate-600'}`}>Excel</button>
                  </div>
                </div>
              </div>
            </div>
