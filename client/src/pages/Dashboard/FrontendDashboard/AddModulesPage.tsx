...
const AddModulesPage = () => {
  ...
  // Drives the "Start Free Trial" CTA on the Professional upgrade card —
  // both calls hit HostPanel's own backend (not MasterPanel directly), which
  // proxies/reads the shared plan data. freeTrialEnabled is the master
  // toggle (Master Panel > Plan Pricing); hasUsedTrial/isTrialing are this
  // company's own one-time-claim state.
  useEffect(() => {
    let active = true;
  ...
  const handleStartTrial = async () => {
    try {
      ...
      // Professional modules just unlocked across the workspace — a reload
      // is the simplest way to get every module card, the sidebar, and this
      // page's own plan gating all in sync with the new plan.
