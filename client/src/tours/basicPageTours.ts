...
const BASIC_PAGE_TOURS: TourRoute[] = [
  ...
  },
  {
    ...
    version: 1,
    title: "Profile",
    description: "Your account area. Each card opens one part of your profile — identity, security, assets, salary slips, or exit requests.",
    steps: [
      { text: "Company Profile", title: "Company Profile", description: "Review the workspace's registered identity — company details, logo, contact information, and the current plan. Visible to Founders, Super Admins, and HR managers." },
      { text: "My Profile", title: "My Profile", description: "Your personal and employment record. Update contact details, change your profile photo, and review work information assigned by HR." },
      { text: "Change Password", title: "Change Password", description: "Replace your login password. Your current password is verified first, then the new one must meet the listed strength rules." },
      { text: "Assigned Assets", title: "Assigned Assets", description: "See every asset allocated to you, open its full details, and raise an issue when something is damaged or not working." },
      { text: "Payslips", title: "Payslips", description: "Every payslip generated for you, with amounts, delivery status, and PDF view and download actions." },
      { text: "Resignation Request", title: "Resignation Request", description: "Submit a resignation request, list the company documents you will return, and track HR's approval status through to your last working day." },
    ],
  ...
  },
  {
    ...
    replayHint: true,
    steps: [
      { selector: 'button[title="Upload company logo"], button[title="Preview company logo"]', title: "Company logo", description: "Selects a new company logo (or previews the current one first). After saving, the updated logo is used in the HostPanel header and supported company-facing surfaces." },
      { text: "Upgrade Plan?", title: "Request a plan upgrade", description: "Opens the upgrade options available from your current plan. Choose a plan and send the request to the master panel for approval; the request status appears under the company information.", side: "left" },
      { selector: '[data-tour="company-profile-info"]', title: "Synced company information", description: "These company, unit, timezone, currency, and plan values are read-only here because they are synchronized from the active workspace setup." },
      { text: "Danger Zone", title: "Danger Zone", description: "Delete your own account. Your record is kept for company history but you can no longer log in. Founders deleting their account also removes every team member's access, so it requires typing DELETE to confirm.", side: "top", align: "center" },
    ],
  ...
  },
  {
    ...
    version: 2,
    title: "My profile",
    description: "Review and maintain your own workspace identity — profile photo, personal contact details, and the employment record HR keeps for you.",
    replayHint: true,
    steps: [
      { selector: '[data-tour="my-profile-avatar"]', title: "Your profile photo", description: "Select the camera button to upload a new photo (up to 2MB). You can crop it before saving, and select the current photo to preview it in full size." },
      { selector: '[data-tour="my-profile-personal"]', title: "Personal & contact details", description: "Your name, email, phone, addresses, location, and emergency contacts. Select Edit on this card when something needs updating." },
      { text: "Edit", exactText: true, title: "Edit your details", description: "Opens the edit dialog for personal and contact fields. Name and email are locked here because they are managed with your login account." },
      { textOnly: true, title: "Inside the edit dialog", description: "Pick the country dial code before entering your phone number so it is validated correctly. Choosing Country reloads State, then City, and a switch copies your current address to permanent. Save applies the changes after validation." },
      { selector: '[data-tour="my-profile-work"]', title: "Work details", description: "Employee ID, role, department, job title, joining date, manager, shift, notice period, and probation are maintained by HR and read-only here. Tenant-company members see their company and designation instead." },
      { text: "Company SOPs", title: "Company documents", description: "On plans that include departments, active company-wide SOPs and policies published by HR appear here for viewing and download.", side: "top" },
      { selector: '[data-tour="my-profile-department-sops"]', title: "Department documents", description: "SOPs and policies scoped to your departments. Founders, Super Admins, and HR managers see every department grouped; other members see only what belongs to or is shared with their own department.", side: "top" },
    ],
  ...
  },
  {
    ...
    description: "Securely verify your current password and replace it with a stronger one.",
    formDescription: "Verify your current password first — the new-password fields stay locked until verification succeeds.",
    steps: [
      { selector: '[data-tour="change-password-panel"]', title: "Update password", description: "The change happens in two stages: verify the current password, then set and confirm the new one. Fields unlock progressively as each stage completes." },
      { text: "Verify", exactText: true, title: "Verify current password", description: "Checks your current password against the account. When it matches, a green Verified badge appears and the remaining fields unlock.", side: "left" },
      { text: "Verified", exactText: true, title: "Verification confirmed", description: "This badge confirms your current password was accepted. The current-password field locks to prevent accidental edits." },
      { selector: 'button[title="Show password"], button[title="Hide password"]', title: "Show or hide password", description: "Each field has its own eye toggle so you can check what you typed without leaving it visible." },
      { selector: '[data-tour="change-password-requirements"]', title: "Password requirements", description: "The new password needs at least 8 characters with upper and lower case letters, plus at least one number or special character. The Submit button stays disabled until verification is done." },
      { text: "Submit", exactText: true, title: "Change the password", description: "Validates the strength rules and confirmation match, then replaces the account password and signs the requirement checklist off." },
    ],
    matches: exact("/profile/change-password"),
  },
  {
    id: "basic-assigned-assets",
    version: 1,
    title: "Assigned assets",
    description: "Review every asset allocated to you, inspect its full record, and report problems with it.",
    recordsDescription: "Only assets assigned directly to you appear here, with their owning department and assigned quantity.",
    steps: [
      { selector: '[data-tour="assigned-assets-search"] input', title: "Find an asset", description: "Filter your assets by name or asset number. Results narrow as you type." },
      { selector: '[data-tour="assigned-assets-cards"]', title: "Your asset cards", description: "Each card shows the asset number, name, category, owning department, how many units are assigned to you, and its status — Active, In Maintenance, or Decommissioned." },
      { text: "View", exactText: true, title: "Open full details", description: "Shows the complete read-only record: serial number, brand/model, location, purchase date, and assignment details." },
      { text: "Raise Issue", exactText: true, title: "Report a problem", description: "Opens an issue form linked to this asset — describe what is wrong, pick a priority, and it goes to the owning department's queue for repair or replacement.", side: "top" },
    ],
    matches: exact("/profile/assigned-assets"),
  },
  {
    id: "basic-payslips",
    version: 1,
    title: "Payslips",
    description: "Every payslip generated for you in one place, with delivery status and PDF access.",
    recordsDescription: "Each row shows the pay period, file, amount, generation and email-delivery dates, plus view and download actions.",
    replayHint: true,
    steps: [
      { selector: '[data-tour="payslips-year-filter"]', title: "Choose the year", description: "Filters everything on the page — summary cards and the payslip list — to one year, or select All Years to see the complete history." },
      { selector: '[data-tour="payslips-summary"]', title: "Year at a glance", description: "Payslips Generated counts the slips produced for the selected year, and Total Net Pay sums their net amounts in your workspace currency." },
      { selector: '[data-tour="payslips-search"] input', title: "Search payslips", description: "Find a slip by month, year, or file name. Results update as you type." },
      { selector: '[data-tour="payslips-table"]', title: "Payslip records", description: "Each row shows the period (for example 1st to 31st Jan 2026), the PDF file name, net amount, and status: Pending while payroll prepares it, Generated once created, Sent after it is emailed to you." },
      { selector: 'button[title="View Payslip"]', title: "View a payslip", description: "Opens the PDF in a new browser tab. Actions are disabled until the file has been generated by Finance." },
      { selector: 'button[title="Download PDF"]', title: "Download the PDF", description: "Saves the payslip PDF to your device using its original file name.", side: "top", align: "center" },
    ],
    matches: exact("/profile/payslips"),
  },
  {
    id: "basic-resignation-request",
    version: 1,
    title: "Resignation request",
    description: "Submit your resignation, declare the company property you will return, and track HR's decision through to your last working day.",
    formDescription: "Explain your reason for leaving clearly — HR reviews every request before approving it.",
    replayHint: true,
    steps: [
      { selector: '[data-tour="resignation-summary"]', title: "Request summary", description: "Counts of your requests by state: Total submitted, Pending awaiting HR review, Approved, and Completed once your exit process finishes." },
      { selector: '[data-tour="resignation-new-request"]', title: "Start or withdraw the form", description: "Toggles the submission form. Only one active resignation can exist, so the button is disabled while a request is Pending or Approved — you'll see a notice instead." },
      { selector: '[data-tour="resignation-form"]', title: "The submission form", description: "Write the reason for leaving (required), tick every company item you will return — ID card, laptop, access card, phone, credit card, insurance card, keys, other assets — and add document notes if needed." },
      { text: "Submit Request", exactText: true, title: "Send it to HR", description: "Submits the request for HR review. Cancel closes the form and clears what you entered. The submit button unlocks only when a reason is provided.", side: "left" },
      { selector: '[data-tour="resignation-table"]', title: "Track your requests", description: "Each row shows the request code, reason, declared documents, status, notice-period length, last working day, and submission date. Rejected requests display HR's reason beneath yours.", side: "top" },
    ],
    matches: exact("/profile/resignation-request"),
  },
  {
  ...
  },
  {
    ...
    autoStart: false,
    title: "Housekeeping",
    description: "Run daily housekeeping — schedule tasks, let bookings trigger cleaning automatically, manage your housekeeping staff, and track history and performance.",
    steps: [
      { selector: '[data-tour="admin-housekeeping-tabs"]', title: "Five views", description: "Housekeeping Staff manages your team, Daily Scheduled Tasks holds your recurring plan, Booking-Based Tasks are generated automatically from room reservations, Task History archives completed work, and Staff Performance tracks completion and on-time rates." },
      { selector: '[data-tour="admin-housekeeping-summary"]', title: "At a glance", description: "These cards summarize whichever tab is open — task counts on the task tabs, staff/attendance/leave counts on Housekeeping Staff, and team averages on Staff Performance." },
      { selector: '[data-tour="admin-housekeeping-filters"]', title: "Filter this view", description: "Every tab has its own filters here — task status, history month and year, employee status, attendance state, or leave status, depending on what's open." },
      { selector: '[data-tour="admin-housekeeping-search"]', title: "Search", description: "Jump straight to a task, area, or employee by name instead of scanning the list." },
      { tabPage: "staff", selector: '[data-tour="admin-housekeeping-staff-subtabs"]', title: "Housekeeping Staff", description: "Employee List shows everyone onboarded to housekeeping, Attendance handles their daily clock-in and clock-out, and Leave Requests tracks and approves their time off." },
      { tabPage: "staff", selector: '[data-tour="admin-housekeeping-staff-list"]', title: "Employee directory", description: "Every housekeeping employee onboarded through HR, with job title, department, phone, and status. View or edit each record from here.", side: "top" },
      { tabPage: "staff", selector: '[data-tour="admin-housekeeping-staff-attendance"]', title: "Staff attendance", description: "Check a housekeeping employee in, start or end their break, check them out, or open their full attendance history on their behalf." },
      { tabPage: "staff", selector: '[data-tour="admin-housekeeping-apply-leave-btn"]', title: "Apply leave for staff", description: "Submit a leave request on behalf of a housekeeping employee, from the Leave Requests sub-tab." },
      { tabPage: "staff", selector: '[data-tour="admin-housekeeping-staff-leave"]', title: "Leave requests", description: "Every leave request raised by your housekeeping staff, with type, dates, days, and status. Select View to open the full request.", side: "top" },
      { tabPage: "scheduled", selector: '[data-tour="admin-housekeeping-add-btn"]', title: "Create a scheduled task", description: "Add Scheduled Task plans a cleaning job with a location, assignee, and start time. Leave Repeats Daily on and it keeps running every day unchanged until you edit it here; turn it off for a one-time task." },
      { tabPage: "scheduled", selector: '[data-tour="admin-housekeeping-scheduled-table"]', title: "Scheduled task board", description: "Tasks show location, floor/wing, assignee, and start time. Change status directly from the dropdown, or view, edit, or cancel a task; anything left unfinished from a prior day is flagged Overdue instead of disappearing.", side: "top" },
      { tabPage: "auto", selector: '[data-tour="admin-housekeeping-auto-tasks"]', title: "Booking-based tasks", description: "These tasks are generated automatically whenever a meeting room booking needs cleaning. Assign a staff member, check off any requested requirements, and mark the task done once it's complete." },
      { tabPage: "history", selector: '[data-tour="admin-housekeeping-history-table"]', title: "Task history", description: "Every completed task for the selected month and year, including both scheduled and booking-based work, with start and end times.", side: "top" },
      { tabPage: "performance", selector: '[data-tour="admin-housekeeping-performance-table"]', title: "Staff performance", description: "Compare each housekeeping employee's assigned versus completed tasks, completion rate, on-time rate, and pending workload.", side: "top" },
    ],
  ...
  },
  {
    ...
    version: 1,
    title: "Printouts",
    description: "Submit printout requests to the front desk, track their status, and edit or cancel your own pending requests before the team starts working on them.",
    recordsDescription: "Each request shows the print ID, document title, copies, priority, attachments, current status, submitted date, and the actions available for that state.",
    steps: [
      { selector: '[data-tour="printouts-summary"]', title: "Your request counts", description: "These cards summarize every printout request you can see, split by Pending, In Progress, Rejected, and Completed so the current queue is easy to scan." },
      { selector: '[data-tour="printouts-status-filters"]', title: "Filter by status", description: "Switch between All, Pending, In Progress, Completed, Rejected, or Cancelled requests without leaving the page." },
      { selector: '[data-tour="printouts-search"]', title: "Find a request", description: "Search by document title or print ID. Results update as you type." },
      { selector: '[data-tour="printouts-new-request"]', title: "Create a printout request", description: "Opens the request form. Describe the document, set copies, priority, color, paper size, double-sided preference, attach up to five files, and submit it to the front desk.", side: "left" },
      { selector: '[data-tour="printouts-table"]', title: "Request records and actions", description: "Each row shows the request details, attachments, status, submitted date, and actions. Use View to inspect the full request; Edit and Cancel appear only while your own request is still Pending.", side: "top" },
    ],
    matches: exact("/common-modules/printouts"),
  },
  {
    id: "basic-printout-management",
    version: 1,
    autoStart: false,
    title: "Printout Management",
    description: "Manage every printout request submitted by internal staff and tenant companies. Switch between the active queue and history, then select Guide to replay the walkthrough for the current view.",
    replayHint: true,
    recordsDescription: "Rows show who requested the printout, whether it came from an internal user or tenant company, the current status, and the actions available to the front desk.",
    steps: [
      { selector: '[data-tour="printout-management-tabs"]', title: "Requests and history", description: "Printout Requests keeps open work plus anything resolved today. Printout History stores completed or rejected requests from previous days." },
      { selector: '[data-tour="printout-management-summary"]', title: "Queue totals", description: "The summary changes with the tab: today's request workload on Requests, and all-time completed, rejected, recent, internal, and tenant totals on History." },
      { selector: '[data-tour="printout-management-status-filters"]', title: "Filter by workflow state", description: "Narrow the current tab by status. History only shows Completed and Rejected filters because open requests stay in the Requests tab." },
      { selector: '[data-tour="printout-management-search"]', title: "Search requests", description: "Find a request by title, print ID, or requester name." },
      { selector: '[data-tour="printout-management-add-request"]', title: "Log a front-desk request", description: "Add Request opens the walk-in form. Pick an internal employee or tenant company employee from real records, then enter the printout details for someone who came directly to the desk.", side: "left" },
      { selector: '[data-tour="printout-management-table"]', title: "Work the queue", description: "Use View for full details. Pending requests can be accepted or rejected; In Progress requests can open uploaded documents for printing and then be marked completed. Resolved records move into History after today.", side: "top" },
    ],
    matches: exact("/department-accesses/administration-department/printouts"),
  },
  {
    id: "basic-dashboard",
    version: 1,
    ...
    steps: [
      ...
      { selector: '[data-notification-trigger]', title: "Unit notifications", description: "Open notifications to see new activity and updates that may need your attention." },
