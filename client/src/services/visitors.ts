...
export const getVisitorManagementOverview = async () => {
  const response = await axiosPrivate.get("/api/visitors/overview");
  return unwrap(response);
};

export const createVisitorLog = async (payload: Record<string, any>) => {
  const response = await axiosPrivate.post("/api/visitors", payload);
  return unwrap(response);
};

export const checkInVisitorLog = async (visitorId: string, payload: Record<string, any> = {}) => {
  const response = await axiosPrivate.patch(`/api/visitors/${visitorId}/check-in`, payload);
  return unwrap(response);
};

export const checkOutVisitorLog = async (visitorId: string, payload: Record<string, any> = {}) => {
  const response = await axiosPrivate.patch(`/api/visitors/${visitorId}/check-out`, payload);
  return unwrap(response);
};
