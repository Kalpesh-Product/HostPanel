...
const AccessTree = ({ clickState = true, autoExpandFirst = false }: AccessTreeProps) => {
  ...
  const allowedUserIds = [
    "67b83885daad0f7bab2f184f",
    "67b83885daad0f7bab2f1852",
    "681a10b13fc9dc666ede401c",
    "67b83885daad0f7bab2f1864",
    "68400714c51ffdef8b5e7e04",
    "67f761c45e8224c532f8fa80",
    "67b83885daad0f7bab2f188b",
    "67b83885daad0f7bab2f1870",
  ];

  const filterHierarchy = (node: any): any => {
    if (!node) return null;
  ...
  const handleSelectUser = (user: any, level: number) => {
  ...
  if (isPending) {
    return (
      ...
      >
        <CircularProgress sx={{ color: "#1E3D73" }} />
  ...
  return (
    <div className="flex flex-col items-center p-6 pt-10 min-h-[30vh]">
      <HierarchyCard
        user={hierarchy}
        handleSelectUser={(user: any) => handleSelectUser(user, 0)}
        isTopLevel={true}
        click={clickState}
      />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-4">
            {user.subordinates.map((subordinate: any) => (
              <HierarchyCard
                ...
                user={subordinate}
                click={clickState}
                handleSelectUser={(nextUser: any) =>
                  handleSelectUser(nextUser, index + 1)
                }
              />
            ))}
          </div>
