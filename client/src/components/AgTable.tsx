...
interface AgTableProps {
  ...
  checkAll?: boolean;
  disabled?: boolean;
  handleBatchAction?: (rows: Array<Record<string, any>>) => void;
...
const AgTableComponent = ({
  ...
  const showLoadingMessage = loading && (!filteredData || filteredData.length === 0);
  const noRowsMessage = showLoadingMessage ? "Loading Data" : "No Rows To Show";
    ...
    <div className="border-b-[1px] border-borderGray">
      ...
      <div className={`flex ${search ? "justify-between" : "justify-end"} items-center py-2`}>
        {search ? (
          <TextField
            ...
            placeholder="Search"
            InputProps={{
              startAdornment: <IoIosSearch size={20} style={{ marginRight: 8 }} />,
            }}
          />
        ) : null}
        <div className="flex items-center gap-4">
      ...
      {batchButton ? (
        <div className="hidden">
          <PrimaryButton title={batchButton} handleSubmit={handleActionClick} />
        </div>
      ) : null}
    </div>
  );
...
const AgTable = memo(AgTableComponent);
