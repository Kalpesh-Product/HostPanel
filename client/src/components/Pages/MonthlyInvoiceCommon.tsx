...
const MonthlyInvoiceCommon = () => {
    ...
    queryFn: async () => {
      try {
        ...
        return Array.isArray(res.data?.allBudgets)
          ? res.data.allBudgets.filter((data: any) => data.isPaid === "Paid")
          : [];
      } catch (err) {
  ...
  const mappedRows = useMemo(() => {
    ...
    return hrFinance.map((item: any, index: number) => {
      ...
      return {
        ...
        finance: {
          ...
          particulars: Array.isArray(finance.particulars) ? finance.particulars : [],
        },
        unit,
      };
    });
      ...
      {viewModal && viewDetails && (
        ...
        >
          <div className="space-y-3">
            <DetalisFormatted title="Expense Name" detail={viewDetails?.expanseName || "-"} />
            <DetalisFormatted title="Expense Type" detail={viewDetails?.expanseType || "-"} />
            <DetalisFormatted title="Department" detail={viewDetails?.department || "-"} />
            <DetalisFormatted title="Unit" detail={viewDetails?.unitName || "-"} />
            <DetalisFormatted title="Unit No" detail={viewDetails?.unitNo || "-"} />
            <DetalisFormatted title="Building" detail={viewDetails?.buildingName || "-"} />
            <DetalisFormatted
              title="Due Date"
              detail={
                viewDetails?.dueDate
            ...
            <DetalisFormatted
              ...
              detail={
                viewDetails?.invoiceDate
                  ? new Date(viewDetails.invoiceDate).toLocaleDateString("en-IN", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })
            ...
            />
            <DetalisFormatted title="Status" detail={viewDetails?.isPaid || "Unpaid"} />
            <DetalisFormatted
              title="Invoice File"
              detail={
                viewDetails?.invoiceLink !== "-" ? (
                  <a href={viewDetails.invoiceLink} target="_blank" rel="noopener noreferrer" className="text-primary underline">
            ...
            {viewDetails?.finance && (
              <div className="mt-4 flex flex-col gap-4">
                ...
                <DetalisFormatted title="Finance Sr No" detail={viewDetails.finance.fSrNo || "-"} />
                <DetalisFormatted title="Mode of Payment" detail={viewDetails.finance.modeOfPayment || "-"} />
                <DetalisFormatted title="Cheque No" detail={viewDetails.finance.chequeNo || "-"} />
                <DetalisFormatted
                  title="Cheque Date"
                  detail={
                    viewDetails.finance.chequeDate
                      ? new Date(viewDetails.finance.chequeDate).toLocaleDateString("en-IN", {
                          day: "2-digit",
                          month: "short",
                          year: "numeric",
                        })
                ...
                <DetalisFormatted
                  ...
                  detail={
                    viewDetails.finance.approvedAt
                      ? new Date(viewDetails.finance.approvedAt).toLocaleDateString("en-IN", {
                          day: "2-digit",
                          month: "short",
                          year: "numeric",
                        })
                ...
                <DetalisFormatted
                  ...
                  detail={
                    viewDetails.finance.expectedDateInvoice
                      ? new Date(viewDetails.finance.expectedDateInvoice).toLocaleDateString("en-IN", {
                          day: "2-digit",
                          month: "short",
                          year: "numeric",
                        })
                ...
                />
                <DetalisFormatted title="Advance Amount" detail={`₹${viewDetails.finance.amount || 0}`} />
                <DetalisFormatted
                  title="Voucher File"
                  detail={
                    viewDetails.finance.voucher?.link ? (
                      <a href={viewDetails.finance.voucher.link} target="_blank" rel="noopener noreferrer" className="text-primary underline">
                ...
                />
                {(viewDetails.finance.particulars || []).map((p: any, idx: number) => (
