...
const PrimaryButton = ({
  ...
  padding,
  className,
  isPending, // New prop for showing the spinner
}) => {
  return (
    <motion.button
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.9 }}
      disabled={disabled || isPending} // Disable if loading
      type={type}
      className={` flex items-center justify-center gap-2 ${
        disabled || isPending ? "bg-gray-400 cursor-not-allowed" : "bg-primary"
    ...
    >
      {isPending && <CircularProgress size={16} color="#1E3D73" />}{" "}
      {/* Spinner */}
