    ...
    const startChatFromQuestion = (flowId: string, question: string) => {
        ...
        setMessages([
            ...
            },
            {
                id: Date.now() + 1,
                role: 'assistant',
                content: 'Sure. Here are a few follow-up questions that can help me guide you better.',
            },
        ]);
        setShowSupportOption(false);
    };

    const handleFollowUpClick = (question: string, index: number) => {
        setMessages((prev) => [
            ...prev,
            {
                id: Date.now() + index,
        ...
        if (index >= 1) {
            setMessages((prev) => [
                ...prev,
                {
                    ...
                    role: 'assistant',
                    content: 'If you still need help, you can contact customer support.',
                },
            ]);
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                {activeFlow ? (
                                    <div className="mt-2 flex flex-col gap-2">
                                        {activeFlow.followUpQuestions.map((question, index) => (
                                            <button
                                                key={question}
                                                type="button"
                                                onClick={() => handleFollowUpClick(question, index)}
                                                className="w-full rounded-xl border border-blue-200 bg-white p-3 text-left text-sm text-gray-800 shadow-sm transition hover:border-blue-400 hover:bg-blue-50/30"
                                            >
                                                {question}
                                            </button>
                                        ))}
