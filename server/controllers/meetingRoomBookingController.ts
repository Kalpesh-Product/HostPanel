    ...
    try {
        ...
        const booking = await createBookingWithUniqueCode({
            ...
            requirements: Array.isArray(req.body.requirements)
                ? req.body.requirements.filter((item: unknown) => typeof item === "string" && item.trim())
                : [],
        });

        // Auto-generate a housekeeping cleaning task for the booked room so
        // it shows up in the Booking-Based Tasks tab; any requested items
        // (water bottles, etc.) become a checklist on the task. This must
        // never block booking creation, so failures are swallowed here.
        try {
            ...
            await HousekeepingTask.create({
                ...
                ownerId: resolvedOwnerId,
                taskNumber,
                taskCode: `HKT-${String(taskNumber).padStart(4, "0")}`,
                taskName: `${room.name} Cleaning`,
                ...
                bookingStartTime: startParts.time,
                bookingEndTime: endParts.time,
                dueAt: range.end,
                status: "Pending",
                isAutoGenerated: true,
                requirements: (booking.requirements || []).map((label: string) => ({ label, fulfilled: false })),
            });
        } catch (housekeepingErr) {
            console.error("Failed to auto-create housekeeping task for booking:", housekeepingErr);
        }
