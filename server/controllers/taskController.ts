...
const isRecurringTaskType = (value = "") => String(value || "").trim().toLowerCase() === "recurring";
...
const getNextTaskNumber = async (workspaceId) => {
  const last = await Task.findOne({ workspaceId }).sort({ taskNumber: -1 }).select("taskNumber").lean();
  return (last?.taskNumber || 0) + 1;
};

const ensureDailyRecurringTaskInstances = async (workspaceId) => {
  const today = startOfDay();
  const latestPerSeries = await Task.aggregate([
    {
      $match: {
        workspaceId: new mongoose.Types.ObjectId(String(workspaceId)),
        isRecurring: true,
        recurringFrequency: "daily",
        recurringTemplateId: { $ne: null },
      },
    },
    { $sort: { scheduledDate: -1, createdAt: -1 } },
    { $group: { _id: "$recurringTemplateId", latest: { $first: "$$ROOT" } } },
  ]);

  for (const series of latestPerSeries) {
    const latest = series.latest;
    const latestDay = latest?.scheduledDate ? startOfDay(latest.scheduledDate) : null;
    const endDay = latest?.dueDate ? startOfDay(latest.dueDate) : null;
    if (!latestDay || !endDay || latestDay.getTime() >= today.getTime() || today.getTime() > endDay.getTime()) {
      continue;
    }

    const taskNumber = await getNextTaskNumber(workspaceId);
    await Task.create({
      workspaceId,
      ownerId: latest.ownerId,
      taskNumber,
      taskCode: `TSK-${String(taskNumber).padStart(4, "0")}`,
      type: latest.type,
      title: latest.title,
      description: latest.description,
      departmentId: latest.departmentId || null,
      raisedBy: latest.raisedBy,
      raisedByUserId: latest.raisedByUserId || null,
      raisedByDeptId: latest.raisedByDeptId || null,
      assignee: latest.assignee || "Unassigned",
      assigneeUserId: latest.assigneeUserId || null,
      priority: latest.priority || "Medium",
      status: "Pending",
      progress: 0,
      dueDate: latest.dueDate,
      startDate: latest.startDate || latest.createdAt || latest.scheduledDate || null,
      isRecurring: true,
      recurringFrequency: "daily",
      recurringTemplateId: series._id,
      scheduledDate: today,
...
export async function createTask(req, res, next) {
  try {
    ...
    if (!resolvedTaskNumber) {
      resolvedTaskNumber = await getNextTaskNumber(workspaceId);
    }
    ...
    const isRecurring = isRecurringTaskType(type);
    const assignedStartDate = startOfDay(new Date());
    const scheduledDate = isRecurring ? assignedStartDate : null;
    ...
    const doc = await Task.create({
      ...
      isRecurring,
      recurringFrequency: isRecurring ? "daily" : "",
      scheduledDate,
      attachments: Array.isArray(attachments) ? attachments : [],
      comments: Array.isArray(comments) ? comments : [],
    });

    if (isRecurring && !doc.recurringTemplateId) {
      doc.recurringTemplateId = doc._id;
      await doc.save();
    }
...
export async function listTasks(req, res, next) {
  try {
    ...
    await ensureDailyRecurringTaskInstances(workspaceId);
