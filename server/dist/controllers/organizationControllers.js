...
export const getOrganizationOverview = async (req, res, next) => {
    try {
        ...
        const members = await WorkspaceMember.find({ workspace: workspace._id })
            ...
            .lean()
            .exec();
        const activeDepartments = (workspace.organizationDepartments || []).filter((department) => department?.isActive !== false);
        const departments = activeDepartments.map((department) => {
            const departmentName = String(department?.name || "").trim().toLowerCase();
            const departmentMembers = members.filter((member) => (Array.isArray(member.departments) ? member.departments : []).some((name) => String(name || "").trim().toLowerCase() === departmentName));
        ...
        const teamMembers = members.map((member) => ({
            ...
            workspaceAccesses: linkedWorkspaces.filter((item) => linkedMemberships.some((accessMembership) => toId(accessMembership.workspace) === String(item.id) &&
...
export const inviteOrganizationMember = async (req, res, next) => {
    try {
        ...
        await WorkspaceMember.findOneAndUpdate({ workspace: workspace._id, user: targetUser._id }, {
            $set: {
                role,
        ...
        const inviteSecret = process.env.HOST_INVITE_TOKEN_SECRET ||
            process.env.REGISTER_INVITE_SECRET ||
            process.env.ACCESS_TOKEN_SECRET;
        const frontendBase = String(process.env.FRONTEND_PROD_LINK || process.env.FRONTEND_DEV_LINK || "http://localhost:5173")
            .trim()
            .replace(/\/$/, "");
...
export const assignOrganizationActingManager = async (req, res, next) => {
    try {
        ...
        const assignments = Array.isArray(workspace.actingManagerAssignments)
            ...
            : [];
        const existing = assignments.find((assignment) => toId(assignment?.departmentId) === toId(department?._id) &&
...
export const removeOrganizationActingManager = async (req, res, next) => {
    try {
        ...
        const assignments = Array.isArray(workspace.actingManagerAssignments)
            ? workspace.actingManagerAssignments
            : [];
        const target = assignments.find((assignment) => toId(assignment?.departmentId) === departmentId &&
            toId(assignment?.assignedUser) === assignedUserId &&
            assignment?.isActive !== false);
        if (target) {
...
export const transferOrganizationMember = async (req, res, next) => {
    try {
        ...
        if (!targetWorkspaceId)
            return res.status(400).json({ message: "Target workspace is required." });
        const member = await WorkspaceMember.findOne({
            _id: req.params.memberId,
            workspace: workspace._id,
            isActive: true,
        ...
        const targetDepartments = requestedDepartmentIds
            ...
            .filter(Boolean);
        await WorkspaceMember.findOneAndUpdate({ workspace: targetWorkspace._id, user: member.user }, {
            $set: {
                role: nextRole,
...
export const transferOrganizationOwnership = async (req, res, next) => {
    try {
        ...
        if (previousOwner) {
            ...
            await WorkspaceMember.findOneAndUpdate({ workspace: workspace._id, user: previousOwner._id }, { $set: { role: "super_admin", isActive: true, status: "joined" } }, { upsert: true, new: true, setDefaultsOnInsert: true });
        }
        nextOwner.role = "owner";
        await nextOwner.save();
        await WorkspaceMember.findOneAndUpdate({ workspace: workspace._id, user: nextOwner._id }, { $set: { role: "owner", isActive: true, status: "joined" } }, { upsert: true, new: true, setDefaultsOnInsert: true });
