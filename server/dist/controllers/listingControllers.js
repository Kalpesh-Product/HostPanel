...
export const createCompanyListing = async (req, res) => {
    try {
        ...
        //   reviews,
        // } = payload;
        const { companyId, companyType, ratings, totalReviews, companyName, companyTitle, cost, description, latitude, longitude, inclusions, about, address, reviews, } = req.body;
        let parsedReviews;
        ...
        const safeCompanyName = (company.companyName || "unnamed").replace(/[^\w\- ]+/g, "").trim() ||
            "unnamed";
        ...
        if (req.files?.length > 0) {
            ...
            if (imageFiles.length > 0) {
                ...
                const sanitizeFileName = (name) => String(name || "file")
                    .replace(/[/\\?%*:|"<>]/g, "_")
                    .replace(/\s+/g, "_");
                const results = await Promise.allSettled(imageFiles.map((file, i) => {
                    const uniqueKey = `${folderPath}/images/${sanitizeFileName(file.originalname)}`;
        ...
        try {
            const response = await axios.post("https://wononomadsbe.vercel.app/api/company/create-company", listingData);
...
export const editCompanyListing = async (req, res) => {
    try {
        ...
        //   existingImages = [],
        // } = payload;
        const { businessId, companyId, companyTitle, companyType, ratings, totalReviews, productName, cost, description, latitude, longitude, inclusions, about, address, reviews, existingImages = [], } = req.body;
        ...
        if (req.files?.length) {
            ...
            if (imageFiles.length) {
                const sanitize = (name) => String(name || "file")
                    .replace(/[/\\?%*:|"<>]/g, "_")
                    .replace(/\s+/g, "_");
        ...
        try {
            const response = await axios.patch("https://wononomadsbe.vercel.app/api/company/update-company", updateData);
            ...
            console.error("❌ Remote update failed:", err.response?.data || err.message);
            // If remote update fails, delete the newly uploaded images to maintain consistency
            if (req.files?.length) {
                const imageFiles = req.files.filter((f) => f.fieldname === "images");
                if (imageFiles.length) {
                    console.log("🧹 Cleaning up newly uploaded images due to remote failure...");
                    const newlyUploadedUrls = updateData.images.slice(existingImages.length);
                    await Promise.allSettled(newlyUploadedUrls.map((img) => deleteFileFromS3ByUrl(img.url)));
                }
            }
...
export const activateProduct = async (req, res, next) => {
    try {
        ...
        const response = await axios.patch("https://wononomadsbe.vercel.app/api/company/activate-product", {
            businessId,
            status,
        });
...
export const getAllCompanyListings = async (req, res) => {
    try {
        const response = await axios.get("https://wononomadsbe.vercel.app/api/company/companies");
        if (!response.data) {
            return res.status(200).json([]);
        }
