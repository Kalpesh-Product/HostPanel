...
export const login = async (req, res, next) => {
    try {
        ...
        const accessToken = jwt.sign({ userInfo: { ...user } }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "15m" });
        const refreshToken = jwt.sign({ userInfo: { ...user } }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: "15d" });
...
export const resetPassword = async (req, res) => {
    try {
      ...
      <p>You can now <a href="${process.env.FRONTEND_PROD_LINK}" target="_blank">log in</a> with your new password.</p>
    ...
    `;
        try {
            ...
            console.error("⚠️ Password reset confirmation email failed:", error.message);
        }
        res.status(200).json({
            success: true,
...
export const startRegisterWithOtp = async (req, res, next) => {
    try {
        ...
        await Otp.updateMany({ email: inviteEmail, purpose: "registration", isUsed: false }, { $set: { isUsed: true } });
        await Otp.create({
            email: inviteEmail,
            code: otp,
    ...
    try {
        ...
        if (existing) {
            return res
                .status(409)
                .json({
                message: "This email already exists in host panel. Use a different email for new account.",
            });
        }
        const otp = `${Math.floor(100000 + Math.random() * 900000)}`;
        await Otp.updateMany({ email: normalizedEmail, purpose: "registration", isUsed: false }, { $set: { isUsed: true } });
        await Otp.create({
            email: normalizedEmail,
            code: otp,
            purpose: "registration",
...
export const verifyRegisterOtpDirect = async (req, res, next) => {
    try {
        ...
        if (!company?._id || !company?.companyId) {
            return res.status(400).json({
                message: "No company found to attach this user. Please create at least one company first.",
            });
        }
        const fallbackCompanyId = `${company.companyId}-dev-${Date.now()
            .toString()
