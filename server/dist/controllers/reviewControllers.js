...
const REVIEW_API_BASE_URL = process.env.REVIEW_API_BASE_URL || "https://wononomadsbe.vercel.app";
export const updateReviewStatus = async (req, res, next) => {
    try {
        ...
        let response;
        try {
            const response = await axios.post(`https://wononomadsbe.vercel.app/api/reviews/${reviewId}`, data);
            // response = await axios.patch(
            //   `http://localhost:3000/api/review/${reviewId}`,
            //   data,
            // );
            response = await axios.patch(`${REVIEW_API_BASE_URL}/api/review/${reviewId}`, data);
...
export const getReviewsByCompany = async (req, res, next) => {
    try {
        ...
        try {
            ...
            const hostMap = Object.fromEntries(hosts.map((h) => [h._id.toString(), h]));
            enrichedReviews = reviews.map((r) => ({
                ...r,
                approvedBy: r.approvedBy
                    ? {
                        ...r.approvedBy,
                        user: r.approvedBy.userType === "MASTER"
                ...
                rejectedBy: r.rejectedBy
                    ? {
                        ...r.rejectedBy,
                        user: r.rejectedBy.userType === "MASTER"
