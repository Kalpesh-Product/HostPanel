...
const getUserContext = async (userId, workspaceId) => {
    ...
    try {
        ...
        if (req.file) {
            const ext = String(req.file.originalname || "")
                ...
                .pop()
                ?.toLowerCase();
            const imageRoute = `support-tickets/${Date.now()}-${Math.round(Math.random() * 1e9)}.${ext || "png"}`;
            uploadedImage = await uploadFileToS3(imageRoute, req.file);
        }
        const normalizedRole = String(context?.membership?.role || "")
            .trim()
            .toLowerCase();
        const isTopRole = normalizedRole === "founder" || normalizedRole === "super_admin";
...
export const followUpSupportTicket = async (req, res) => {
    try {
        ...
        const newTicket = await SupportTicket.create({
            ...
            workspaceName: context?.workspace?.workspaceName || ticket.workspaceName || "",
