...
export const createTemplate = async (req, res, next) => {
    try {
        ...
        const validateTextFields = () => {
            ...
            if (Array.isArray(about)) {
                for (const [index, item] of about.entries()) {
                    const error = enforceMaxLength(`About item ${index + 1}`, item, TEXT_LIMITS.aboutItem);
                    if (error)
                        return error;
                }
            }
            if (Array.isArray(products)) {
                for (const [index, product] of products.entries()) {
                    const typeError = enforceMaxLength(`Product ${index + 1} type`, product?.type, TEXT_LIMITS.productType);
                    if (typeError)
                        return typeError;
                    const nameError = enforceMaxLength(`Product ${index + 1} name`, product?.name, TEXT_LIMITS.productName);
                    if (nameError)
                        return nameError;
                    const costError = enforceMaxLength(`Product ${index + 1} cost`, product?.cost, TEXT_LIMITS.productCost);
                    if (costError)
                        return costError;
                    const descriptionError = enforceMaxLength(`Product ${index + 1} description`, product?.description, TEXT_LIMITS.productDescription);
                    if (descriptionError)
            ...
            if (Array.isArray(testimonials)) {
                for (const [index, testimonial] of testimonials.entries()) {
                    const nameError = enforceMaxLength(`Testimonial ${index + 1} name`, testimonial?.name, TEXT_LIMITS.testimonialName);
                    if (nameError)
                        return nameError;
                    const jobError = enforceMaxLength(`Testimonial ${index + 1} job position`, testimonial?.jobPosition, TEXT_LIMITS.testimonialJob);
                    if (jobError)
                        return jobError;
                    const testimonyError = enforceMaxLength(`Testimonial ${index + 1} testimony`, testimonial?.testimony, TEXT_LIMITS.testimonialText);
                    if (testimonyError)
                        return testimonyError;
                }
            }
            return null;
        };
        ...
        const hostCompanyExists = await HostCompany.findOne({ companyName: req.body.companyName });
        if (!hostCompanyExists && source !== "Nomad") {
            return res.status(400).json({ message: "Company not found" });
        }
        for (const k of Object.keys(req.body)) {
            if (/^(products|testimonials)\.\d+\./.test(k))
                delete req.body[k];
        }
        ...
        if (req.body.testimonials) {
            ...
            template.testimonials = parsedTestimonials.map((t) => t?.url ? { url: t.url, id: t.id } : {});
        }
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (pFiles.length > 10) {
                return res.status(400).json({
                    message: `Max 10 images allowed per product (${products[i].name || "Unnamed product"}).`,
                });
            }
        }
        // Gallery: max 40
        ...
        if (filesByField.heroImages?.length) {
            template.heroImages = await uploadImages(filesByField.heroImages, `${baseFolder}/heroImages`);
        }
        // gallery
        if (filesByField.gallery?.length) {
            template.gallery = await uploadImages(filesByField.gallery, `${baseFolder}/gallery`);
        }
        if (Array.isArray(products) && products.length) {
            for (let i = 0; i < products.length; i++) {
                const p = products[i] || {};
                const pFiles = filesByField[`productImages_${i}`] || [];
                const uploaded = await uploadImages(pFiles, `${baseFolder}/productImages/${i}`);
        ...
        if (filesByField.testimonialImages?.length) {
            ...
            tUploads = await uploadImages(filesByField.testimonialImages, `${baseFolder}/testimonialImages`);
        }
        else {
            // Back-compat: testimonialImages_${i}
            for (let i = 0; i < testimonials.length; i++) {
                const tFiles = filesByField[`testimonialImages_${i}`] || [];
                const uploaded = await uploadImages(tFiles, `${baseFolder}/testimonialImages/${i}`);
                tUploads[i] = uploaded[0]; // one file per testimonial
            }
        }
        // template.testimonials = (testimonials || []).map((t, i) => ({
        template.testimonials = (Array.isArray(testimonials) ? testimonials : []).map((t, i) => ({
        ...
        if (source !== "Nomad") {
            const updateHostCompany = await HostCompany.findOneAndUpdate({ companyName: req.body.companyName }, //can't use company Id as the host signup form can't send any company Id
            {
                isWebsiteTemplate: true,
            });
            ...
            try {
                const updatedCompany = await axios.patch("https://wononomadsbe.vercel.app/api/company/add-template-link", {
                    companyName: req.body.companyName,
                ...
                if (error.response?.status !== 200) {
                    return res.status(201).json({
                        message: "Failed to add link.Check if the company is listed in Nomads.",
                        error: error.message,
                    });
                }
            }
...
export const editTemplate = async (req, res, next) => {
    ...
    try {
        ...
        const validateTextFields = () => {
            ...
            if (Array.isArray(about)) {
                about.forEach((item, index) => {
                    enforceMaxLength(`About item ${index + 1}`, item, TEXT_LIMITS.aboutItem);
                });
            }
            if (Array.isArray(products)) {
                products.forEach((product, index) => {
                    enforceMaxLength(`Product ${index + 1} type`, product?.type, TEXT_LIMITS.productType);
                    enforceMaxLength(`Product ${index + 1} name`, product?.name, TEXT_LIMITS.productName);
                    enforceMaxLength(`Product ${index + 1} cost`, product?.cost, TEXT_LIMITS.productCost);
                    enforceMaxLength(`Product ${index + 1} description`, product?.description, TEXT_LIMITS.productDescription);
                });
            }
            if (Array.isArray(testimonials)) {
                testimonials.forEach((testimonial, index) => {
                    enforceMaxLength(`Testimonial ${index + 1} name`, testimonial?.name, TEXT_LIMITS.testimonialName);
                    enforceMaxLength(`Testimonial ${index + 1} job position`, testimonial?.jobPosition, TEXT_LIMITS.testimonialJob);
                    enforceMaxLength(`Testimonial ${index + 1} testimony`, testimonial?.testimony, TEXT_LIMITS.testimonialText);
                });
            }
        };
        validateTextFields();
        ...
        if (filesByField.companyLogo?.length) {
            ...
            const uploaded = await uploadImages([filesByField.companyLogo[0]], `${baseFolder}/companyLogo`, 1);
        ...
        if (req.body.heroImageIds !== undefined) {
            const toDelete = template.heroImages.filter((img) => !heroKeepIds.includes(img.id));
            await deleteImagesFromS3(toDelete);
            template.heroImages = template.heroImages.filter((img) => heroKeepIds.includes(img.id));
        }
        ...
        if (totalHeroCount > 5) {
            throw new Error(`Cannot exceed 5 hero images (currently ${template.heroImages.length}).`);
        }
        if (newHeroFiles.length) {
            const newHero = await uploadImages(newHeroFiles, `${baseFolder}/heroImages`, 5);
        ...
        if (req.body.galleryImageIds !== undefined) {
            const toDelete = template.gallery.filter((img) => !galleryKeepIds.includes(img.id));
            await deleteImagesFromS3(toDelete);
            template.gallery = template.gallery.filter((img) => galleryKeepIds.includes(img.id));
        }
        ...
        if (totalGalleryCount > 40) {
            throw new Error(`Cannot exceed 40 gallery images (currently ${template.gallery.length}).`);
        }
        if (newGalleryFiles.length) {
            const newGallery = await uploadImages(newGalleryFiles, `${baseFolder}/gallery`, 40);
            template.gallery.push(...newGallery);
        }
        // === 🛍 PRODUCTS (max 10 per product) ===
        const existingMap = new Map((template.products || []).map((p) => [String(p._id), p]));
        const idxById = new Map((template.products || []).map((p, i) => [String(p._id), i]));
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            const fieldIdx = p._id && idxById.has(String(p._id))
            ...
            if (total > 10) {
                throw new Error(`Max 10 images allowed per product (${p.name || "Unnamed product"}).`);
            }
            const uploaded = newFiles.length
                ? await uploadImages(newFiles, `${baseFolder}/productImages/${p._id || fieldIdx}`, 10)
                : [];
            if (existing) {
                const keepIds = new Set(p.imageIds || []);
                if (p.imageIds !== undefined) {
                    const toDelete = (existing.images || []).filter((img) => !keepIds.has(img.id));
                    await deleteImagesFromS3(toDelete);
                    const kept = (existing.images || []).filter((img) => keepIds.has(img.id));
        ...
        const updatedIds = new Set(updatedProducts.map((p) => String(p._id)).filter(Boolean));
        const removedProducts = (template.products || []).filter((p) => !updatedIds.has(String(p._id)));
        ...
        const testimonialMap = new Map((template.testimonials || []).map((t) => [String(t._id), t]));
        const tIdxById = new Map((template.testimonials || []).map((t, i) => [String(t._id), i]));
        ...
        for (let i = 0; i < testimonials.length; i++) {
            ...
            const fieldIdx = t._id && tIdxById.has(String(t._id))
        ...
        const updatedTIds = new Set(updatedTestimonials.map((t) => String(t._id)).filter(Boolean));
        const removedT = (template.testimonials || []).filter((t) => !updatedTIds.has(String(t._id)));
        for (const r of removedT)
            if (r.image?.url)
                await deleteImagesFromS3([r.image]);
