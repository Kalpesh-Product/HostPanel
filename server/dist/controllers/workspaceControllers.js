...
const normalizeStringArray = (value) => Array.isArray(value)
    ...
    : [];
const buildAuthUserPayload = (user, company, workspaceMembership = null, workspaceCount = 1) => ({
    ...user,
...
export const completeWorkspaceSetup = async (req, res, next) => {
    try {
        ...
        const normalizedWorkspaceName = normalizeWorkspaceName(workspaceDetails.workspaceName || "");
        const normalizedBusinessName = String(workspaceDetails.businessName || "").trim();
        ...
        let company = (user.company && (await Company.findById(user.company))) ||
        ...
        const updatedCompany = (company?._id && (await Company.findById(company._id).lean().exec())) ||
            null;
        ...
        return res.status(201).json({
            ...
            workspace,
            user: buildAuthUserPayload(updatedUser, updatedCompany, workspaceMembership, workspaceCount),
        });
    }
    catch (error) {
        next(error);
...
const getCurrentWorkspaceContext = async (userId) => {
...
const canManageWorkspaces = (workspaceMembership) => {
...
const normalizeWorkspaceName = (value) => String(value || "").trim();
const buildWorkspaceNameRegex = (value) => new RegExp(`^${value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i");
...
export const getWorkspaceManagementOverview = async (req, res, next) => {
    try {
        ...
        const list = ownerWorkspaces.map((item) => {
            ...
            const employees = workspaceMembers.map((member) => {
...
export const switchWorkspace = async (req, res, next) => {
    try {
        ...
        await WorkspaceMember.updateOne({ user: user._id, workspace: targetWorkspace._id }, { $set: { isPrimary: true, isActive: true } });
